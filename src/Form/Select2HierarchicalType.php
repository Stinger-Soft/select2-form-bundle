...
class Select2HierarchicalType extends Select2BaseType {
	 ...
	 * @param string $root
	 */
	protected function removeDeadNodes(array &$data, $root) {
		$removeParts = array();
		$this->findDeadNodes($data, $root, array(), $removeParts);
	...
	protected function findDeadNodes(array &$data, $key, array $path = array(), array &$removeParts = array()) {
		$path[] = $key;
		$item = $data[$key];
	 ...
	 */
	protected function addChildrenFlat($entityChoice, array &$data, array $choices, array $options, $level, $inclFullChildren = false) {
			...
			if (array_key_exists($child->getId(), $choices)) {
				...
				if ($inclFullChildren) {
					...
					if (!isset($data[$entityChoice->getId()]['_children'])) {
						$data[$entityChoice->getId()]['_children'] = array();
					}
	 ...
	 */
	protected function addChildren($entityChoice, array &$data, array $choices, array $options, $level, $childrenOption = 'children') {
		$children = $this->propertyAccessor->getValue($entityChoice, $options['children_path']);
		foreach ($children as $child) {
			if (array_key_exists($child->getId(), $choices)) {
	...
	public function getBlockPrefix() {
		return 'stinger_soft_form_hierarchical';
	}
}