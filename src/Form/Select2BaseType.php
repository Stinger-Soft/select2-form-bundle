...
abstract class Select2BaseType extends AbstractType {
	...
	public function buildForm(FormBuilderInterface $builder, array $options) {
		...
		$builder->setAttribute('maximumSelectionLength', $options['maximumSelectionLength']);
		...
		$builder->setAttribute('selectionCssClass', $options['selectionCssClass']);
	...
	public function buildView(FormView $view, FormInterface $form, array $options) {
		...
		$view->vars['select2OptionsJavaScript'] = array();
		$view->vars['select2Options'] = array();
		...
		$maximumSelectionLength = $form->getConfig()->getAttribute('maximumSelectionSize', $form->getConfig()->getAttribute('maximumSelectionSize'));
		$view->vars['select2Options']['maximumSelectionLength'] = $maximumSelectionLength;
		...
		$view->vars['select2Options']['selectionCssClass'] = $form->getConfig()->getAttribute('selectionCssClass');
		...
		if ($form->getConfig()->getAttribute('select2-placeholder')) {
			...
			$placeholder = $translationDomain === false ? $placeholder : $this->translate($placeholder, array(), $translationDomain);
		...
		if (!$form->getConfig()->getRequired() && !isset($view->vars['select2Options']['placeholder'])) {
			$view->vars['select2Options']['placeholder'] = '---';
		}
		if ($form->getConfig()->getAttribute('route')) {
			$view->vars['select2Options']['ajax'] = array();
			...
			$view->vars['select2OptionsJavaScript']['ajax'] = array();
	...
	public function configureOptions(OptionsResolver $resolver) {
		$resolver->setDefault('theme', 'bootstrap5');
		$resolver->addAllowedTypes('theme', array(
			'null',
			'string'
		));

		$resolver->setDefault('searchMethod', self::SEARCH_METHOD_EXACT);
		$resolver->addAllowedTypes('searchMethod', array(
			'integer',
			'string'
		));
		$resolver->setAllowedValues('searchMethod', function ($value) {
			if (is_string($value)) {
				return true;
			}
		...
		$resolver->addAllowedTypes('formatResult', array(
			'null',
			'integer',
			'string'
		));
		$resolver->setAllowedValues('formatResult', function ($value) {
			if (!$value)
				return true;
			if (is_string($value)) {
				return true;
		...
		$resolver->addAllowedTypes('templateResult', array(
			'null',
			'string'
		));

		$resolver->setDefault('templateSelection', null);
		$resolver->addAllowedTypes('templateSelection', array(
			'null',
			'string'
		));
		...
		$resolver->setDefault('attr', function (Options $options, $definedValue) {
			if ($definedValue) {
				return $definedValue;
			}
		...
		$resolver->setDefault('hideSelected', function (Options $options, $previousValue) {
		...
		$resolver->setDefault('containerCssClass', 'form-select');
		$resolver->addAllowedTypes('containerCssClass', 'string');
		$resolver->setDefault('selectionCssClass', ':all:');
		$resolver->addAllowedTypes('selectionCssClass', 'string');
		...
		$resolver->addAllowedTypes('multiple', array(
			'bool'
		));
		...
		$resolver->addAllowedTypes('route', array(
			'string',
			'bool'
		));
		...
		$resolver->addAllowedTypes('escapeMarkup', array(
			'null',
			'string'
		));
	}
	 ...
	 * @return string
	 */
	protected function translate(string $id, array $parameters, string $domain = 'messages'): string {
